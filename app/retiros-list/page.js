...
export default function ProductList() {
  ...
  const redirectEgresos = () => {
    ...
    const newUrl = currentUrl.replace('/retiros-list', '/retiro');
    window.location.href = newUrl;
  };

  const redirectIngresosList = () => {
    const currentUrl = window.location.href;
    const newUrl = currentUrl.replace('/retiros-list', '/ingresos-list');
    window.location.href = newUrl;
  };

  const redirectInventario = () => {
    const currentUrl = window.location.href;
    const newUrl = currentUrl.replace('/retiros-list', '/inventario');
    window.location.href = newUrl;
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between sm:p-24 p-4">
      <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm">
        <h1 className="text-4xl p-4 text-center">Lista de Retiros</h1>
