...
export default function Home() {
    ...
    if (newItem.name !== '' && newItem.name !== 'Seleccione' && newItem.quantity !== '') {
      ...
      try {
        ...
        if (productDoc.exists()) {
            ...
            const quantityToAdd = parseInt(newItem.quantity, 10);
  ...
  const redirectEgresosList = () => {
    ...
    const newUrl = currentUrl.replace('/ingreso-prod', '/retiros-list');
    window.location.href = newUrl;
  };

  const redirectIngresosList = () => {
    const currentUrl = window.location.href;
    const newUrl = currentUrl.replace('/ingreso-prod', '/ingresos-list');
    window.location.href = newUrl;
  };

  const redirectInventario = () => {
    const currentUrl = window.location.href;
    const newUrl = currentUrl.replace('/ingreso-prod', '/inventario');
    window.location.href = newUrl;
  };

  const redirectEgresos = () => {
    const currentUrl = window.location.href;
    const newUrl = currentUrl.replace('/ingreso-prod', '/retiro');
    window.location.href = newUrl;
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between sm:p-24 p-4 bg-white">
      <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm">
        <h1 className="text-4xl p-4 text-black text-center">Ingreso de mercadería</h1>
        ...
        <div className="flex justify-around w-full mt-4">
          ...
          <button 
            ...
            className="text-white bg-red-600 hover:bg-red-700 p-4 text-l rounded-lg">
            Inventario
          </button> 
        </div>
      </div>
